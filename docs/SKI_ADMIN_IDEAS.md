...
Here are practical AI additions for the admin dashboard, aligned with your Ski Admin vision (file://Ski Admin \- Ideas.pdf):

* Demand & capacity prediction: forecast program/subprogram fill rates and waitlist risk; flag which offerings to expand or close.  
* Pricing & scholarship guidance: suggest price tweaks or early-bird windows based on historical conversion and seasonality; highlight families likely needing assistance (opt-in, privacy-safe).  
* Roster & staffing recommendations: recommend coach-to-athlete ratios and volunteer needs per session based on registrations and attendance history.  
* Communication copilot: draft segmented emails (e.g., Alpine Devo parents, waitlisted families) with tone and length controls; summarize key dates and fees for quick send.  
* Waiver & compliance summarizer: condense what’s outstanding by household/athlete; generate reminders and deadlines.  
* Payment & churn signals: flag households at risk of churn (no registrations yet, unpaid orders, prior drop-offs) with suggested outreach.  
* Race & event readiness: auto-build race rosters from eligibility (age/program) and past participation; surface conflicts and gaps.  
* Data hygiene assistant: detect duplicate households/athletes, inconsistent ages/grades, and missing guardian contact details; propose merges/fixes.  
* Coach insights: summarize athlete participation trends and attendance anomalies per coach/program; draft action items to share.  
* Admin Q\&A: natural-language query over clubs/seasons/programs (“Show unpaid orders for Jackson U12 this month”) with saved prompt shortcuts.

Implementation notes:

* Start with read-only insights (no auto-changes); pair every AI action with “view reasoning” and “apply/ignore” buttons.  
* Respect privacy: keep prompts data-minimized (no PII beyond what’s needed), and allow per-club feature gating.  
* Phase rollout: begin with summaries and demand forecasts; add generation (emails), then decision support (pricing, staffing).
...
1. **users** (Supabase Auth table \+ profile table)  
   * id, email, name, role (`parent`, `coach`, `admin`, `master_admin`), etc.  
2. **clubs**  
   * id, name, slug, logo\_url, primary\_color, address.  
3. **seasons**  
   * id, club\_id, name (`2025/26`), start\_date, end\_date, status (active, archived).  
4. **households**  
   * id, club\_id, primary\_guardian\_id, address, phone, emergency\_contact.  
5. **household\_guardians**  
   * join table: household\_id, user\_id, is\_primary.  
6. **athletes**  
   * id, household\_id, first\_name, last\_name, dob, gender, medical\_notes.  
7. **sports**  
   * id, club\_id, season\_id, name (Alpine, Nordic, etc.), status.  
8. **programs**  
   * id, sport\_id, season\_id, name (Devo, Prep), description, base\_price, capacity, status.  
9. **subprograms**  
   * id, program\_id, name (2x/week), price\_delta or price, capacity, waitlist\_enabled, status.  
10. **registrations**  
    * id, athlete\_id, season\_id, sport\_id, program\_id, subprogram\_id, status (`pending`, `confirmed`, `waitlisted`, `cancelled`), price.  
11. **orders**  
    * id, household\_id, total\_amount, status (`unpaid`, `paid`, `partially_paid`, `cancelled`), created\_at.  
12. **order\_items**  
    * id, order\_id, registration\_id, description, amount.  
13. **payments**  
    * id, order\_id, amount, method (`stripe`, `cash`, `check`), status, stripe\_payment\_intent\_id.  
14. **waivers**  
    * id, club\_id, season\_id, title, body (text), required (bool).  
15. **waiver\_signatures**  
    * id, waiver\_id, athlete\_id, guardian\_id, signed\_at, signed\_name.
...
* **Message / Communication**  
  ...
  * `target_segment` (filters), status, sent\_at.  
...
* Steps:  
  1. **Account:** Email, password, name, phone, consent to terms/privacy.  
  2. **Household:** Address, emergency contact, additional guardian (optional).  
  3. **First Athlete:** Name, DOB, gender, medical/allergy info.  
  4. **Club Waiver:** Show relevant season waiver; require signature (check \+ type name \+ date; or redirect to DocuSign).  
...
* Tabs:  
  1. **Profile:** DOB, medical notes, membership IDs, equipment notes.  
  2. **Registrations:** list of seasons \+ programs; status (confirmed/waitlisted).  
  3. **Documents:** Waivers status for this athlete; re-sign if new season.  
  4. **Races (v2):** upcoming and past races for this athlete.
...
1. Dashboard  
2. Setup (Club Settings, Seasons, Waivers, Payment & Email settings)  
3. People (Households, Athletes, Coaches, Admins)  
4. Programs (Sports, Programs, Subprograms, Capacity/Waitlists)  
5. Registrations & Orders  
6. Communication  
7. Calendar & Races  
8. Reports
...
1. **Revenue vs Last Season (Chart)**  
...
2. **Registrations by Sport/Program (Chart)**  
