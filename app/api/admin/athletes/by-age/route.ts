...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await admin.rpc('get_athletes_by_age', {
  ...
  if (error) {
    console.error('Athletes by age error:', error)
    return NextResponse.json(
      { error: error.message || 'Failed to load athletes by age' },
      { status: 500 }
    )
  }

  const distribution: AgeGroupDistribution[] = (data || []).map((row: any) => ({
